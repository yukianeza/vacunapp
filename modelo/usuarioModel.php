...
class usuario {
    ...
    public function getUsuario() {
        $this->sql = "SELECT trabajador.* FROM trabajador INNER JOIN persona ON persona.id = trabajador.idpersona ORDER BY id DESC";
        $this->queryBD();
        $data = array();
        $temp = array();
        if ($this->result->num_rows > 0) {
            ...
            while($row = $this->result->fetch_assoc()) {
                $temp["id"] =  $row["id"]; 
                $temp["persona"] =  $row["idpersona"];  
                $temp["puesto"] =  $row["puesto"];    
                $temp["turno"] =  $row["Turno"]; 
                $temp["usuario"] =  $row["usuario"]; 
                $temp["clave"] =  $row["clave"]; 

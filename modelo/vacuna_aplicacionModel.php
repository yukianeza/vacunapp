...
class VacunaAplicacion {
    ...
    public function getVacunaAplicacion() {
        $this->sql = "SELECT vacuna_aplicacion.id, vacuna_id, vacuna.nombre AS vacuna, aplicacion_id, aplicacion.detalle as aplicacion FROM vacuna_aplicacion
                        INNER JOIN vacuna on vacuna_id = vacuna.id
                        INNER JOIN aplicacion on aplicacion_id = aplicacion.id ";
        $this->queryBD();
        $data = array();
        $temp = array();
        if ($this->result->num_rows > 0) {
            ...
            while($row = $this->result->fetch_assoc()) {
                $temp["id"] =  $row["id"]; 
                $temp["vacuna"] =  $row["vacuna"];  
                $temp["aplicacion"] =  $row["aplicacion"];    
